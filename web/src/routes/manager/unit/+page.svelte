<script lang="ts">
  ...
  if (searchQuery.trim() !== "") {
    ...
    filtered = filtered.filter((unit) => {
      ...
      const matchesTenantName = unit.current_lease?.tenants?.some((tenant: { first_name: any; last_name: any; }): boolean => {
  ...
  // apply sorting
  filtered.sort((a, b) =>
    sortAscending ? a.unit_number - b.unit_number : b.unit_number - a.unit_number
  );
