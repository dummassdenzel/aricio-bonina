<script lang="ts">
  ...
  function toggleDropdown() {
      dropdownOpen = !dropdownOpen; // user dropdown logout
  }

  function handleLogout() {
      auth.logout();
      goto("/web/"); // redirect to landing page
  }
...
<header class="flex justify-between">
  ...
  <nav class="flex items-center gap-12 bg-back rounded-full p-3 px-10">
      <!-- dashboard -->
      <a href="/web/manager/dashboard" 
         class="font-semibold text-xs flex items-center gap-2"
         on:click={() => setActiveLink('dashboard')}
         class:text-teal={activeLink === 'dashboard'}
         class:text-muted={activeLink !== 'dashboard'}>
          <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke={activeLink === 'dashboard' ? '#314A60' : '#686868'} stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-layout-dashboard">
      ...
      <!-- unit -->
      <a href="/web/manager/unit" 
         class="font-semibold text-xs flex items-center gap-2"
         on:click={() => setActiveLink('unit')}
         class:text-teal={activeLink === 'unit'}
         class:text-muted={activeLink !== 'unit'}>
          <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke={activeLink === 'unit' ? '#314A60' : '#686868'} stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-bed-single">
      ...
      <!-- tenants -->
      <a href="/web/manager/tenant" 
         class="font-semibold text-xs flex items-center gap-2"
         on:click={() => setActiveLink('tenant')}
         class:text-teal={activeLink === 'tenant'}
         class:text-muted={activeLink !== 'tenant'}>
          <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke={activeLink === 'tenant' ? '#314A60' : '#686868'} stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-users-round">
      ...
      <!-- billing -->
      <a href="/web/manager/billing" 
         class="font-semibold text-xs flex items-center gap-2"
         on:click={() => setActiveLink('billing')}
         class:text-teal={activeLink === 'billing'}
         class:text-muted={activeLink !== 'billing'}>
          <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke={activeLink === 'billing' ? '#314A60' : '#686868'} stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-wallet">
